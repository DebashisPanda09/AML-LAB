...
def calculate():
    ...
    slopes = {name: f"{coef:.4f}" for name, coef in zip(x_names, coefficients)}
...
def predict():
    ...
    for i, feature in enumerate(feature_strings):
        x_data[f'x{i + 1}'] = [float(x) for x in feature.split(",") if x.strip()]
    ...
    try:
        ...
        for i in range(len(feature_strings)):
            sample_dict[f'x{i + 1}'] = float(request.form[f'pred_x_{i}'])
