...
y_vals = np.array([float(x) for x in y_input.split(",") if x.strip()])
...
for idx in range(num_features):
    feat_name = input(f"Enter the name of feature x{idx + 1} : ").strip()
    x_names.append(feat_name)

    feat_input = input(f"Enter values for {feat_name} (x{idx + 1}): ")
    feat_vals = np.array([float(x) for x in feat_input.split(",") if x.strip()])
...
if not validation_passed:
    ...
    for i, coef in enumerate(coefficients):
        print(f"Slope for {x_names[i]} (b{i + 1}): {coef:.4f}")
    ...
    if choice in ("yes", "y"):
        try:
            ...
            for i, name in enumerate(x_names):
                sample_dict[f"x{i + 1}"] = float(input(f"Enter value for {name} (x{i + 1}): "))
    ...
    min_val = min(np.min(y_vals), np.min(predictions))
    max_val = max(np.max(y_vals), np.max(predictions))
